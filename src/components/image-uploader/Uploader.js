...
const Uploader = ({ setImageUrl, imageUrl }) => {
	...
	return (
		<div className="w-full text-center">
			...
			<aside className="flex flex-row flex-wrap mt-4">{imageUrl ? <img className="inline-flex border rounded-md border-gray-100 w-24 max-h-24 p-2" src={imageUrl} alt="product" /> : thumbs}</aside>
		</div>
	);
};
